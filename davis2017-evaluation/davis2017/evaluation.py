...
class DAVISEvaluation(object):
    ...
    @staticmethod
    def _evaluate_unsupervised(all_gt_masks, all_res_masks, all_void_masks, metric, max_n_proposals=300):
        ...
        js, fs = [], []
        all_metrics = j_metrics_res + f_metrics_res # n k t
        for t in range(all_metrics.shape[2]):
            row_ind, col_ind = linear_sum_assignment(-all_metrics[:,:,t])
            j = j_metrics_res[row_ind, col_ind, t]
            f = f_metrics_res[row_ind, col_ind, t]
            js.append(j)
            fs.append(f)
        js = np.stack(js, axis=-1)
